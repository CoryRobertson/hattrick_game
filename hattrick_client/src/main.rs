...
async fn main() {
    ...
    loop {
        ...
        let ping = SystemTime::now().duration_since(local_gs.clone().time).unwrap();
        let ping_color = {
            if ping.as_millis() > 16 { RED }else { GREEN }
        };
        draw_text(&*format!("Ping: {:.2}ms", ping.as_secs_f64() * 1000.0), 10., 10., 18., ping_color);
...
fn spawn_connect_thread(game_state:Arc<Mutex<GameState>>, running: Arc<Mutex<bool>>) -> JoinHandle<()> {
    thread::spawn(move || {
